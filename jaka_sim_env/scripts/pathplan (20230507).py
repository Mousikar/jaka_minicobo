...
class PointCloudSubscriber(object):
    ...
    def callBack(self,pcd_msg):
        ...
        # print(pts_in_camera[1])
        # o3d.visualization.draw_geometries([pcd_in_camera,mesh_frame])
        #save the cropped region
        # np.savetxt( '1_' + '.txt', xyz_load)

        # # publish
        # header = Header()
        # header.frame_id = "d435_depth_frame" #pcd_msg.header.frame_id
        ...
        for i in range(len(xyz_load)):
            # pts_in_link0.append(np.dot(rotation.T,xyz_load[i,:])+np.array([0,0,0]))
            # pts_in_link0.append(xyz_load[i,:]+
            #                         np.array([
            #                             0, 
            #                             2, 
            #                             0]))
            pts_in_link0.append(np.dot(rotation.T,xyz_load[i,:])
                                        ...
                                        0.2175, 
                                        1.5]))
            # pts_in_link0.append(np.dot(rotation.T,xyz_load[i,:])+np.array([-0.013694490066100018, -1.4999995243977522,0.21750000000000003 ]))#-np.dot(rotation.T,np.array([1.4999995243977522, -0.21750000000000003, -0.013694490066100018]).T))
